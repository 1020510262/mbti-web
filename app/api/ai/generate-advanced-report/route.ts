...
import { ProfileInput } from "@/lib/ai/report-types";
...
export async function POST(req: NextRequest) {
  ...
  const limit = checkLimit(sessionId, "generate_advanced", 3, 10 * 60_000);
  ...
  if (order.productType !== "advanced") return jsonErr(2003, "invalid order product for advanced", 400);

  const existingKey = `${orderNo}:advanced`;
  ...
  try {
    ...
    record.errorMessage = error instanceof Error ? error.message : "generate advanced failed";
    record.updatedAt = nowIso();
    await persistStore();
